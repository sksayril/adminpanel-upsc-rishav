...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push("/");
      }, 1800); // Wait for the success animation to finish
    } catch (err) {
  ...
  const handleDemoLogin = async () => {
    ...
    setIsSubmitting(true);
    try {
      await loginSimulate("admin@demo.com", "demo12345");
      setIsSuccess(true);
      setTimeout(() => {
        router.push("/");
      }, 1800);
      ...
      <div className="w-full max-w-md bg-white/95 rounded-[2.5rem] border border-white/20 premium-shadow p-8 md:p-10 relative overflow-hidden backdrop-blur-md animate-slide-up z-10">
        ...
        {isSuccess && (
          <div className="absolute inset-0 bg-white z-20 flex flex-col items-center justify-center p-8 animate-fade-in">
            ...
            <h3 className="text-xl font-bold text-slate-800 mt-6 tracking-tight">Success!</h3>
            <p className="text-xs text-slate-400 mt-2 font-medium">Redirecting to Dashboard...</p>
          </div>
        )}
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-5">
          {/* Error Message */}
          {error && (
            <div className="bg-rose-50 border border-rose-100 text-rose-600 text-xs font-bold rounded-xl px-4 py-3 animate-shake">
              {error}
            </div>
          )}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-full text-xs font-bold text-slate-700 bg-slate-50 border border-slate-200/80 rounded-2xl pl-12 pr-4 py-4 focus:bg-white focus:border-[#5113C2] focus:ring-4 focus:ring-indigo-100 outline-none transition-all duration-300 shadow-inner disabled:opacity-70"
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex justify-between items-center pl-1">
              ...
              <button
                type="button"
                className="text-[10px] font-bold text-[#5113C2] hover:text-[#7B3FE4] transition-colors outline-none cursor-pointer"
              >
                Forgot?
              </button>
            </div>
            <div className="relative flex items-center">
              ...
              <input
                ...
                placeholder="••••••••"
                disabled={isSubmitting}
                className="w-full text-xs font-bold text-slate-700 bg-slate-50 border border-slate-200/80 rounded-2xl pl-12 pr-12 py-4 focus:bg-white focus:border-[#5113C2] focus:ring-4 focus:ring-indigo-100 outline-none transition-all duration-300 shadow-inner disabled:opacity-70"
              />
              <button
                type="button"
          ...
          <div className="flex items-center gap-2 pl-1 mt-1">
            <input
              ...
              id="remember"
              disabled={isSubmitting}
              className="w-4.5 h-4.5 rounded border-slate-200 accent-[#5113C2] cursor-pointer"
            />
            <label htmlFor="remember" className="text-[10px] font-bold text-slate-400 uppercase select-none cursor-pointer">
              Remember my session
            </label>
          </div>
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className="w-full bg-[#F5C518] hover:bg-[#E5B612] text-[#42169B] font-extrabold text-xs py-4 px-6 rounded-2xl shadow-md transition-all duration-300 active:scale-98 flex items-center justify-center gap-2 mt-2 cursor-pointer disabled:opacity-75 disabled:cursor-not-allowed"
          >
            {isSubmitting ? (
              <>
          ...
          <button
            ...
            disabled={isSubmitting}
            className="w-full bg-slate-50 border border-slate-200 text-[#5113C2] hover:bg-slate-100 hover:border-slate-300 font-extrabold text-xs py-4 px-6 rounded-2xl transition-all duration-300 active:scale-98 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-75 disabled:cursor-not-allowed"
          >
